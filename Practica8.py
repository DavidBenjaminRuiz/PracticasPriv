...
def escribeTABSIM(CONTLOC, ETIQUETA, VALOR_EQU, CODOP):
    if codop.strip() != "EQU":
        TABSIM.write( str(ETIQUETA) + "\t" + str(CONTLOC) + "\n")
    else :
        TABSIM.write(str(ETIQUETA) + "\t" + str(VALOR_EQU) + "\n")
...
def codigoMaquinaRel8(tipoRel, operando, direccionOrigen, direccionDestino, codigoMaquina):
...
def IdentificarTipoRelativo(codop):
    if codop.strip().__contains__("L"):
        return 16
    else:
        return 8    
...
for i in range(0, len(lineas)):
    ...
    if llevaOperando(Dataframe , lista) is False and t_OPERANDO(lineas[i], rel, inme, inh).__contains__('1') is False and t_OPERANDO(lineas[i], rel, inme, inh).__contains__('Inherente, Un byte') is False and codop.strip() != "END":
        errNollevaOp=1
    ...
    if str(t_OPERANDO(lineas[i],rel, inme,inh )).__contains__("Inherente"):
        ...
        modoDir= "[IDX2]"
    elif str(t_OPERANDO(lineas[i],rel, inme,inh)).__contains__("REL"):
        modoDir= "REL"
    ...
    operandoDefinitivo=operando[0:11].replace("'","").replace("(","")
    #Detecta el org y incia el contloc
    if codop.strip() == "ORG":
        org=str(limpiaORG(DIR_INIC))
        CONTLOC= int(org,16)
        print(CONTLOC)
    ...
    if codop.strip() != "EQU" and modoDir == "IDX" and indexar=='5' and codop.strip() != "END":
        valorCM= valorCodigoMaquina(Dataframe, lista, modoDir)
        tipoDeRel=0
    ...
    if codop.strip() != "EQU" and modoDir == "IDX1" and indexar=='9' and codop.strip() != "END":
        valorCM= valorCodigoMaquina(Dataframe, lista, modoDir)
        tipoDeRel=0
    ...
    if codop.strip() != "EQU" and modoDir == "IDX2" and indexar=='16' and codop.strip() != "END":
        valorCM= valorCodigoMaquina(Dataframe, lista, modoDir)
        tipoDeRel=0
    ...
    if codop.strip() != "EQU" and modoDir == "IDX" and indexar=='+-' and codop.strip() != "END":
        valorCM= valorCodigoMaquina(Dataframe, lista, modoDir)
        tipoDeRel=0
